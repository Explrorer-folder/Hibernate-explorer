...
@TestInstance(PER_CLASS) // по умолчанию стоит PER_METHOD т.е. будет создаваться новый объект на каждый метод.
// Но тут только select запросы -> изменений в БД нету -> незачем создавать всё заново
class UserDaoTest {
